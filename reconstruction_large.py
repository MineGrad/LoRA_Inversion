...
#print(weight_grad[0][:,t-1])
        ...
        rec.append(weight_grad[0][:,t-1].reshape(1,1024) ) 
        rec[t-1]=torch.div(rec[t-1],weight)
        rec[t-1]=torch.mul(rec[t-1],div_factor)
        rec[t-1]=torch.div(rec[t-1],new_scaling)

        rec[t-1]=(rec[t-1]-w_glob['embedding.position_embeddings'][0][patch_id[t-1]].reshape(1,1024))
...
#print(weight_grad[0][:,t-1])
        ...
        rec.append(weight_grad[:,t-1].reshape(1,1024) ) 
        rec[t-1]=torch.div(rec[t-1],weight)
        rec[t-1]=torch.mul(rec[t-1],div_factor)
        rec[t-1]=torch.div(rec[t-1],new_scaling)
